...
def model_train_sm(loader, model, optimizer, loss_fn, scaler, DEVICE, fake_batch_size=1):
    ...
    for batch_idx, (node_feat, coor, edges, edge_feat, carb_binder, sm_binder, label_res, n_res, n_edge) in enumerate(loop):
        ...
        curr_loss += loss_fn(pred_res,label_res,model)
        #For normal loss fun
    ...
    for batch_idx, (node_feat, coor, edges, edge_feat, carb_binder, sm_binder, label_res, n_res, n_edge) in enumerate(loop):
        ...
        with torch.autograd.set_detect_anomaly(True):
            ...
            if torch.isnan(node_feat).any() or torch.isinf(node_feat).any():
                print("Warning: NaN/Inf in node_feat detected, skipping batch")
                continue
            ...
            #For bayes loss
            curr_loss = loss_fn(pred_prot,label_prot,model)
            # For normal Loss
...
def elbo_loss(pred,true,model,beta=.001):
    ...
    ce_loss = criterion(pred, true)
    #print("CE Loss:",ce_loss)

    #print("KL Loss:",kl)
    loss = ce_loss + kl*beta
...
def dice_pp_loss(pred,true,eps=1e-5,gamma: float = 2):
    pred = torch.squeeze(pred)
    true = torch.squeeze(true)
    ...
    d = (2 * torch.sum(tp) + eps) / ((torch.sum(pred)**gamma) + (torch.sum(true)**gamma) + eps)
    return 1 - d;
